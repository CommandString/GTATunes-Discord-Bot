        ...
        try {
            await player.lock(async () => {
                ...
                > = {
                    ...
                    pause: async (interaction, player) => {
                        if (!player.isPlaying) {
                            await interaction.reply({
                                content:
                                    "The playback isn't currently playing.",
                                flags: MessageFlags.Ephemeral
                            });
                    ...
                    next: async (interaction, player) => {
                        if (!player.currentSong || !player.currentStation) {
                            await interaction.reply({
                                content:
                                    'A song must be playing to use this command.',
                                flags: MessageFlags.Ephemeral
                            });
                ...
                try {
                    ...
                    await interaction.reply({
                        content:
                            'Failed to handle action, please try again later.',
                        flags: MessageFlags.Ephemeral
                    });
                }
            }, interaction);
            ...
            gtaTunesLog(
                'PLAYER',
                `Failed to handle player command in ${interaction.guild?.name} (${interaction.guild?.id})`,
                e
            );
            await interaction.reply({
